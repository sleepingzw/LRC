<template>
  <section class="workspace-asset-view" :data-theme="theme" :aria-label="`素材 ${path}`">
    <header><div><p class="asset-kicker">具体文件</p><h2>{{ path }}</h2><p>{{ asset.role || '其他' }} · {{ formatSize(asset.size ?? pendingFile?.raw?.size) }}</p></div><span>{{ pendingFile ? '待保存' : '已上传' }}</span></header>
    <p v-if="error" role="alert">{{ error }}</p><p v-else-if="loading" role="status">正在加载文件…</p>
    <template v-else-if="file">
      <section v-if="isText" class="asset-text"><MonacoLrcEditor :model-value="text" :language="language" :theme="theme" :read-only="locked" :aria-label="`编辑 ${path}`" @update:model-value="text = $event" /><footer v-if="!locked"><button type="button" :disabled="busy" @click="applyText">应用文本修改</button></footer></section>
...
async function load(){ const version=++request; imageEdit.value=false; error.value=''; loading.value=true; clearUrl(); file.value=null; text.value=''; try { const next=pendingRaw.value || await props.loadAsset?.(props.asset); if(!next) throw new Error('文件不可用'); if(version !== request) return; file.value=next; if(isText.value) { const nextText=await next.text(); if(version !== request) return; text.value=nextText; } else url.value=URL.createObjectURL(next); } catch (reason) { if(version === request) error.value=`加载失败：${reason.message || '无法读取文件'}`; } finally { if(version === request) loading.value=false; } }
...
function applyText(){ if(!file.value || locked.value) return; const next=new File([text.value], filename.value,{type:file.value.type || 'text/plain'}); if(props.pendingFile) updatePending(next); else emit('replace',{asset:props.asset,file:next,path:path.value}); file.value=next; }
function applyImage(next){ imageEdit.value=false; if(props.pendingFile) updatePending(next); else emit('replace',{asset:props.asset,file:next,path:path.value}); file.value=next; clearUrl(); url.value=URL.createObjectURL(next); }
