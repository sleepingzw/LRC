...
describe('文件工作区真实组件流', () => {
  ...
  it('左侧上传直接保存素材并在树中打开具体文件，不再显示旧上传窗口', async () => {
    ...
    await openFile(w, 'new.png'); expect(w.findComponent({ name: 'WorkspaceAssetView' }).exists()).toBe(true);
  });
  it('左侧分片上传失败后从素材总览重试，只重传失败片段并复用编号', async () => {
    ...
    const file = new File(['tiny'], 'big.wav'); Object.defineProperty(file, 'size', { value: DIRECT_UPLOAD_LIMIT + 1 }); await importFile(w, file); expect(w.text()).toContain('保存失败'); expect(calls.some(call => call.path === '/api/workspace/save')).toBe(false);
    await openFile(w, '素材'); await button(w, '保存').trigger('click'); await flushPromises(); expect(creates).toBe(1); expect(parts.filter(n => n === 1)).toHaveLength(1); expect(parts.filter(n => n === 2)).toHaveLength(2); expect(parts.at(-1)).toBe(Math.ceil((DIRECT_UPLOAD_LIMIT + 1) / MULTIPART_PART_SIZE)); expect(saved(calls).assets).toHaveLength(1);
  });
  ...
  it('目录内联新建普通文件与歌词映射，错误保留输入且文件在右侧打开', async () => {
    ...
    const folder = w.findAll('.workspace-tree button').find(node => node.text().endsWith('歌词')); await folder.trigger('click'); await folder.trigger('click'); expect(w.findAll('[role=tab]')).toHaveLength(0);
